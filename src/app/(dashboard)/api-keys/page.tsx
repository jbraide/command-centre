...
// Shared vault keys (same as Password Vault)
const SALT_KEY = 'vault-salt';
const VERIFY_IV_KEY = 'vault-verify-iv';
const VERIFY_TOKEN_KEY = 'vault-verify-token';

// Old separate keys (for migration)
const OLD_SALT_KEY = 'api-key-vault-salt';
const OLD_VERIFY_IV_KEY = 'api-key-vault-verify-iv';
const OLD_VERIFY_TOKEN_KEY = 'api-key-vault-verify-token';

// Helper: get a localStorage key with fallback to old key, migrating if needed
function getVaultKey(key: string, oldKey: string): string | null {
...
function setVaultKey(key: string, value: string) {
  localStorage.setItem(key, value);
  // Clean up old key if it exists
  const oldKey = key === SALT_KEY ? OLD_SALT_KEY : key === VERIFY_IV_KEY ? OLD_VERIFY_IV_KEY : OLD_VERIFY_TOKEN_KEY;
  localStorage.removeItem(oldKey);
}

function removeVaultKeys() {
  [SALT_KEY, VERIFY_IV_KEY, VERIFY_TOKEN_KEY, OLD_SALT_KEY, OLD_VERIFY_IV_KEY, OLD_VERIFY_TOKEN_KEY].forEach(k => localStorage.removeItem(k));
}
...
export default function ApiKeysPage() {
  ...
  useEffect(() => {
    const salt = getVaultKey(SALT_KEY, OLD_SALT_KEY);
    setIsNewVault(!salt);
  }, []);
  ...
  const handleSetupVault = async () => {
    ...
    try {
      ...
      setVaultKey(SALT_KEY, uint8ArrayToBase64(salt));
      setVaultKey(VERIFY_IV_KEY, uint8ArrayToBase64(verifyIv));
      setVaultKey(VERIFY_TOKEN_KEY, verifyToken);
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      const saltB64 = getVaultKey(SALT_KEY, OLD_SALT_KEY);
      if (!saltB64) {
      ...
      const verifyIvB64 = getVaultKey(VERIFY_IV_KEY, OLD_VERIFY_IV_KEY);
      const verifyToken = getVaultKey(VERIFY_TOKEN_KEY, OLD_VERIFY_TOKEN_KEY);
  ...
  const fetchDecrypted = async (id: string): Promise<string | null> => {
    ...
    try {
      ...
      const data: ApiKeyWithSecret & { key?: string } = await res.json();
      
      // Try client-side decryption first (master password)
      try {
        const iv = base64ToUint8Array(data.iv);
        return await decrypt(derivedKey, data.encryptedKey, iv);
      } catch {
        // Client-side decryption failed — try server-decrypted key
